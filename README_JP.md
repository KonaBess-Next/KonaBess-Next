...
**Q: 旧バージョンの KonaBess は動くのに、Next はチップを認識しません。なぜですか？**  
**A:** KonaBess Next は、新しいチップ（8 Gen 2/3, Elite）向けに設計された新しい DTS パーサーを使用して完全に書き直されています。この移行期間中は、古いチップや特定の OEM 向けのサポートが一時的に壊れる可能性があります。

**Q: 私のデバイスはサポートされていますか？**  
**A:** 旧アプリで動作していたなら、*最終的には*動作するようになります。現在は新しいパーサーを安定化させているところなので、古いチップセットでは不安定な動作が予想されます。

**Q: デバイスで動作しません。どうすればいいですか？**  
**A:** 以下の情報を含めて、[GitHub](https://github.com/KonaBess-Next/KonaBess-Next/issues) で issue（問題）を報告してください：
- デバイスモデルと ROM
- チップセット（例：SD865）
- `boot.img`、`dtbo.img`、または `.dtb` ファイル
- エラーのスクリーンショットまたはログ

**Q: 古いバージョンを使い続けたほうがいいですか？**  
**A:** 今すぐ古いデバイスで完全な安定性が必要な場合は、旧アプリを使用してください。Next のマテリアル UI、非 root モード、カーブエディタをいち早く体験したい方は、ぜひテストに協力しバグを報告してください！
